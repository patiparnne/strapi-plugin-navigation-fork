...
export default function adminController(context: { strapi: Core.Strapi }) {
  return {
    ...
    async exportNavigation(ctx: KoaContext) {
      const {
        params: { documentId },
      } = ctx;

      return await this.getAdminService().exportNavigation({
        documentId: idSchema.parse(documentId),
      });
    },

    async importNavigation(ctx: KoaContext & KoaContextExtension) {
      const body = z
        .object({
      ...
      try {
        return await this.getAdminService().importNavigation({
        ...
        return ctx.internalServerError('Unable to import navigation', { originalError });
      }
    },
  };
}
