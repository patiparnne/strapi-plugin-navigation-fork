...
const adminService = (context: { strapi: Core.Strapi }) => ({
  ...
  async configContentTypes({
    ...
    const eligibleContentTypes = await Promise.all(
      config.contentTypes
        ...
        .map(async (key) => {
          const parseResult = contentTypeFullSchema.safeParse(strapi.contentTypes[key as any]);
          if (!parseResult.success) {
            strapi.log.warn(`[Navigation] Skipping content type "${key}" — schema parse failed: ${parseResult.error.issues.map(i => `${i.path.join('.')}: ${i.message}`).join(', ')}`);
            return undefined;
          }
    ...
    return eligibleContentTypes.reduce((acc, current) => {
      ...
      const parseResult2 = contentTypeFullSchema.safeParse(context.strapi.contentTypes[key as any]);
      if (!parseResult2.success) {
        strapi.log.warn(`[Navigation] Skipping content type "${key}" in reduce — schema parse failed`);
        return acc;
      }
      const item = parseResult2.data;
  ...
  async exportNavigation({ documentId }: { documentId: string }): Promise<Record<string, unknown>> {
    const commonService = getPluginService(context, 'common');
    const { defaultLocale, restLocale } = await commonService.readLocale();
    const allLocales = [defaultLocale, ...restLocale];

    const stripIds = (items: NavigationItemDBSchema[]): any[] =>
      items.map(({ id, documentId, parent, items: children, ...rest }) => ({
        ...rest,
        items: children ? stripIds(children) : [],
      }));
    ...
    for (const locale of allLocales) {
      try {
        const navigation = await this.getById({ documentId, locale, populate: ['related'] });
        const { id: _id, documentId: _docId, items, ...rest } = navigation;

        locales.push({
          ...rest,
          items: items ? stripIds(items) : [],
        });
      } catch {
    ...
    // Return with the default locale as the top-level and all locales nested
    const primary = locales[0] as Record<string, unknown>;
    return {
      ...primary,
      localeVersions: locales,
    };
  },
  ...
  async importNavigation({
    ...
    name?: string;
  }): Promise<NavigationDBSchema> {
    const commonService = getPluginService(context, 'common');
    const repository = getNavigationRepository(context);
    const { defaultLocale } = await commonService.readLocale();

    const localeVersions = (nav.localeVersions as any[]) || [nav];

    // Use the first locale version (default) to create the primary navigation
    const primaryNav = localeVersions[0] || nav;
    const name = overrideName || (primaryNav.name as string);
    const visible = (primaryNav.visible as boolean) ?? true;
    const primaryLocale = (primaryNav.locale as string) || defaultLocale;
    const primaryItems = (primaryNav.items as any[]) || [];

    const slug = await commonService.getSlug({ query: name });

    // Create the primary locale navigation
    const newNavigation = await repository.save({
      name,
      visible,
    ...
    // Import items for the primary locale
    if (primaryItems.length) {
      const masterEntity = await repository.findOne({
        filters: { documentId: newNavigation.documentId },
        locale: primaryLocale,
        populate: '*',
      });
      ...
      await commonService.createBranch({
        action: { create: true },
        masterEntity,
        navigationItems: await Promise.all(primaryItems.map(itemProcessor)),
        parentItem: undefined,
      });
    }

    // Import remaining locale versions
    for (let i = 1; i < localeVersions.length; i++) {
      const localeNav = localeVersions[i];
      const locale = localeNav.locale as string;
      const localeItems = (localeNav.items as any[]) || [];

      if (!locale) continue;

      // Create the locale version sharing the same documentId
      await repository.save({
        name,
        visible: (localeNav.visible as boolean) ?? visible,
        locale,
        slug,
        documentId: newNavigation.documentId,
      });
      ...
      if (localeItems.length) {
        ...
        await commonService.createBranch({
          action: { create: true },
          masterEntity,
          navigationItems: await Promise.all(localeItems.map(itemProcessor)),
          parentItem: undefined,
        });
      }
    }

    return await this.getById({ documentId: newNavigation.documentId });
  },
});
