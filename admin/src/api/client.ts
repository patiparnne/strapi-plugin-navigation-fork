...
export const getApiClient = once((fetch: ReturnType<typeof getFetchClient>) => ({
  ...
  exportNavigation(documentId: string) {
    return fetch
      .get(`/${URL_PREFIX}/export/${documentId}`)
      .then(({ data }) => data as Record<string, unknown>);
  },

  importNavigation(body: { navigation: Record<string, unknown>; name?: string }) {
    return fetch
      .post(`/${URL_PREFIX}/import`, body)
      .then(({ data }) => data);
  },
}));
