...
export const useContentTypeItems = (input: UseContentTypeItemsInput) => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      let result = await apiClient.readContentTypeItems(input);
      if (result && result.length === 0 && input.locale) {
        result = await apiClient.readContentTypeItems({ ...input, locale: undefined });
      }
      return result;
    },
    staleTime: 1000 * 60 * 3,
