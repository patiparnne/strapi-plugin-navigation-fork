...
import { ImportNavigation, ImportNavigationFooter } from './ImportNavigation';
...
export const NavigationManager = ({ initialState, isOpened, onClose }: Props) => {
            ...
            : state.view === 'CACHE_PURGE'
              ...
              : state.view === 'IMPORT'
                ? async () => {
                    ...
                    await apiClient.importNavigation({
  ...
  return (
    ...
    >
      <Modal.Content>
        <Modal.Header>
          <Typography variant="omega" fontWeight="bold" textColor="neutral800" tag="h2">
            {header}
          </Typography>
        </Modal.Header>
...
const renderContent = (state: State, setState: SetState, isLoading: boolean) => {
  ...
  switch (state.view) {
    ...
    case 'IMPORT': {
      return <ImportNavigation {...state} {...commonProps} />;
    }
    case 'INITIAL': {
      return <Loader small />;
    }
...
const renderFooter: Footer = (props) => {
  switch (props.state.view) {
    ...
    case 'IMPORT': {
      return <ImportNavigationFooter {...props} disabled={props.state.disabled} />;
    }
    case 'ERROR': {
