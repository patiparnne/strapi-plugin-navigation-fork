...
export const Item: React.FC<Props> = ({
  ...
  // Smart path building: if mappedItem.path looks like a query parameter,
  // or if levelPath ends with =, don't add a leading slash
  const shouldSkipSlash = isQueryParameter(mappedItem.path || '', levelPath);
  
  const absolutePath = isExternal
    ? undefined
    : shouldSkipSlash
      ? `${levelPath === '/' ? '' : levelPath}${mappedItem.path === '/' ? '' : mappedItem.path}`
