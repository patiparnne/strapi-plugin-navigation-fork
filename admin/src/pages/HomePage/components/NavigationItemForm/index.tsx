...
export type { ContentTypeEntity, GetContentTypeEntitiesPayload } from './types';
...
export const NavigationItemForm: React.FC<NavigationItemFormProps> = ({
  ...
  const submit = async (e: React.MouseEvent, values: NavigationItemFormSchema) => {
    ...
    if (success) {
      ...
      // For non-INTERNAL types, allow custom uiRouterKey even if slugification fails
      // since they don't rely on internal routing and Strapi allows custom regex for slugs
      if (!uiRouterKey && payload.type === 'INTERNAL') {
        toggleNotification({
          type: 'warning',
      ...
      // Use the title as fallback for non-INTERNAL types if uiRouterKey generation failed
      const finalUiRouterKey = uiRouterKey || (payload.type !== 'INTERNAL' ? title : '');
